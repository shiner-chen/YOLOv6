# ET-YOLOv6n + NWD loss — ARD100 ROI640 从头训练 (Scratch Training)
# 不使用320预训练，直接在640分辨率上训练
#
# 假设验证：
#   - 640分辨率目标是320的2倍大 (5.83px vs 2.92px)
#   - 320预训练权重针对小尺度，可能造成负迁移
#   - 从头训练可能比迁移学习效果更好
#
# 对比实验：
#   - 320 从头训练: 88.04% mAP@0.5
#   - 640 迁移学习: 83.19% mAP@0.5 @ Epoch 24
#   - 640 从头训练: ??? (本实验)
#
# Training command:
#   torchrun --nproc_per_node=2 --master_port=29530 \
#       tools/train.py \
#       --conf configs/et_yolov6n_ard100_roi640_nwd_scratch.py \
#       --data-path data/ard100_roi640.yaml \
#       --img-size 640 \
#       --batch-size 64 \
#       --epochs 300 \
#       --device 0,1 \
#       --name et_yolov6n_roi640_nwd_scratch
